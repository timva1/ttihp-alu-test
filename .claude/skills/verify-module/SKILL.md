...
3. **Investigate without assuming which side is wrong.** Read waveforms/logs, trace signal values, and form a root-cause hypothesis. The bug is equally likely to be in the RTL (`src/`) or in the verification model/testbench (`test/`) — treat both as suspects until the evidence points one way.
   - For cycle-relationship questions — a strobe/pulse vs. a bus access landing a cycle late, "which cycle did this register actually flip", NBA/edge-sampling races (the recurring failure family in this repo) — get **ground truth from the waveform** rather than reasoning about cocotb scheduling in the abstract. Use `test/tools/vcd_trace.py` to print the relevant signals sampled at each clock edge: first regenerate a real VCD with `make -B test_<module> FST=` (the default dump is FST despite the `.vcd` name), then `python tools/vcd_trace.py waves/<module>_tb.vcd --list` to find hierarchical names and `--clk <clk> --signals <globs> --window START_NS END_NS` to trace the failing window. A tb probe wire (`wire dbg_x = uut.sub.internal;`) exposes internal signals to both the trace and the test. The evidence you paste into step 4 should be an excerpt of that per-edge table, not a prose description of what you think happened.
